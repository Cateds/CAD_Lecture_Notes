...
于是我们可以用一个复数来表示一个正弦信号，这个复数的实部是信号的实部，虚部是信号的虚部。此时 $V_m e^{i(\omega t+\Phi)}=V_m e^{i\Phi}e^{i\omega t}$ 相当于在复平面内的一个半径为 $V_m$ 的圆上逆时针运动，运动的角速度是 $\omega \ \mathrm{rad/s}$ ，我们观测到的信号部分则是它的实部。
